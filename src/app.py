...
app = App(
    name="Line_Message",
    app_type="devops",
    logo="linemessage.svg",
    logo_dark="linemessage_dark.svg",
    product_vendor="Line",
    product_name="Line Message",
    publisher="Joy Chen",
    appid="f5c0cc2c-fcc3-452a-a6e9-8facb4f9951b",
...
def multicast_message(
    ...
    if len(recipients) > 500:
        raise ActionFailure(
            f"Too many recipients ({len(recipients)}). LINE API maximum is 500."
        )
    if len(recipients) > 400:
        logger.warning(
            "Recipient count is %d, approaching the LINE API limit of 500.",
            len(recipients),
        )
...
class UserProfileOutput(ActionOutput):
    ...
    pictureUrl: str = OutputField(
        example_values=["https://profile.line-scdn.net/ch/v2/p/uf9da5ee2b..."]
    )
...
def get_profile(
    ...
    return UserProfileOutput(
        **{k: v for k, v in data.items() if k in UserProfileOutput.model_fields}
    )
...
def get_message_quota(params: Params, soar: SOARClient, asset: Asset) -> QuotaOutput:
    ...
    data = r.json()
    return QuotaOutput(
        **{k: v for k, v in data.items() if k in QuotaOutput.model_fields}
    )
...
def get_message_quota_consumption(
    ...
    return QuotaConsumptionOutput(
        **{k: v for k, v in data.items() if k in QuotaConsumptionOutput.model_fields}
    )
